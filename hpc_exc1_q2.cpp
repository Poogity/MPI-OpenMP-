...
void write_density_mpi_vis(Diffusion2D* D2D, char* filename, int rank, int procs)
{
    ...
    std::stringstream ss;
    int k = rank*real_N_*local_N_;
    for (int i = 0; i < local_N_; ++i){
        for (int j = 0; j < real_N_; ++j) {
            double x_p = i * dr_ - 0.5 * L_;
            double y_p = j * dr_ - 0.5 * L_;
            ss << k << "\t";
            ss << std::fixed << std::setprecision(6) << x_p << "\t";
            ss << std::fixed << std::setprecision(6) << y_p << "\t";
            ss << std::fixed << std::setprecision(6) << rho_[i * real_N_ + j];
            ss << "\n";
            k++;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    if (procs == 1) {
        write_density_vis(&system, (char*)"density_seq_vis.dat");
        write_density(&system, (char*)"density_seq.bin");
    }
    else {
        write_density_mpi(&system, (char*)"density_mpi.bin", rank, procs);
        write_density_mpi_vis(&system, (char*)"density_mpi_vis.dat", rank, procs);
